...
static bool8 MonKnowsOneFieldMove(struct Pokemon *mon)
{
    u32 i,j, move;
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        for (j = 0; sFieldMoves[j] != FIELD_MOVES_COUNT; j++)
        {
            move = GetMonData(mon, i + MON_DATA_MOVE1);
            if (move == sFieldMoves[j])
            {
                return TRUE;
            }
...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    if (!InBattlePike())
    {
        ...
        if (MonKnowsOneFieldMove(&mons[slotId]))
...
static void Task_HandleSelectionMenuInput(u8 taskId)
{
    if (!gPaletteFade.active && MenuHelpers_ShouldWaitForLinkRecv() != TRUE)
    {
        ...
        case MENU_B_PRESSED:
            ...
            sCursorOptions[MENU_CANCEL1].func(taskId);
            break;
        default:
            PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[2]);
...
static void CB2_SelectBagItemToGive(void)
{
    if (InBattlePyramid() == FALSE)
        GoToBagMenu(ITEMMENULOCATION_PARTY, POCKETS_COUNT - 1, CB2_GiveHoldItem);
    else
...
void ItemUseCB_RareCandy(u8 taskId, TaskFunc task)
{
    ...
    if (!(B_RARE_CANDY_CAP && sInitialLevel >= GetCurrentLevelCap(FALSE)))
    {
