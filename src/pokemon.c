...
u32 GetBoxMonData3(struct BoxPokemon *boxMon, s32 field, u8 *data)
{
    ...
    else
    {
        ...
        {
            retVal = boxMon->memory;
            break;
        }
        case MON_DATA_MEMORY_NEW:
        {
            retVal = boxMon->memory2;
            break;
        }
        default:
            break;
        ...
        case 3:
             ...
             && GetMonData(mon, MON_DATA_LEVEL, NULL) != MAX_LEVEL)
            {
                ...
                else if (param - 1 < ARRAY_COUNT(sExpCandyExperienceTable)) // EXP Candies
                {
                    ...
                    if (B_RARE_CANDY_CAP && gSaveBlock2Ptr->optionsLevelCap == OPTIONS_LEVEL_CAPS_HARD)
                    {
...
u8 GetTrainerEncounterMusicId(u16 trainerOpponentId)
{
    if (InBattlePyramid())
        ...
        return GetTrainerArrayFromDifficulty()[SanitizeTrainerId(trainerOpponentId)].encounterMusic_gender & (F_TRAINER_FEMALE - 1);
}
...
bool8 TryIncrementMonLevel(struct Pokemon *mon)
{
    ...
    if (nextLevel > GetCurrentLevelCap(FALSE) || expPoints < gExperienceTables[gSpeciesInfo[species].growthRate][nextLevel])
    {
        return FALSE;
    }
    else
...
const u8 *GetTrainerPartnerName(void)
{
    if (gBattleTypeFlags & BATTLE_TYPE_INGAME_PARTNER)
    {
        ...
        {
            return GetTrainerNameFromId(TRAINER_STEVEN_CHAMPION);
        }
        else
        {
