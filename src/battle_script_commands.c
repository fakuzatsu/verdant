...
static void AccuracyCheck(bool32 recalcDragonDarts, const u8 *nextInstr, const u8 *failInstr, u16 move)
{
    ...
    if (move == NO_ACC_CALC_CHECK_LOCK_ON)
    {
        ...
        || gMovesInfo[move].effect == EFFECT_SPIRIT_CHRONICLE))))
    {
...
static void Cmd_healthbarupdate(void)
{
    ...
    if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT) || (gHitMarker & HITMARKER_PASSIVE_DAMAGE))
    {
        ...
        if (!DN_CONFIG_ONLY_ATTACK_DAMAGE && !gSaveBlock2Ptr->optionsDamageNumsOff)
        ...
        if (gBattleControllerExecFlags == 0)
        {
            ...
            else
            {
                ...
                if (IsValidForBattle(&gPlayerParty[*expMonId]))
                {
                    ...
                    if (gSaveBlock2Ptr->optionsLevelCap == OPTIONS_LEVEL_CAPS_HARD 
                    && gBattleMoveDamage != 0)
                    {
                        ...
                        u32 levelCap = GetCurrentLevelCap(FALSE);
...
static bool32 HasAttackerFaintedTarget(void)
{
    if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
        ...
        || gChosenMove == GetMaxMove(gBattlerAttacker, gChosenMoveByBattler[gBattlerAttacker]) || gChosenMove == GetTeraMove(gBattlerAttacker, gChosenMoveByBattler[gBattlerAttacker])))
        return TRUE;
    else
        return FALSE;
}
